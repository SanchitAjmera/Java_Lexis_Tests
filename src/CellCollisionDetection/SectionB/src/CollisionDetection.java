...
public class CollisionDetection {
     ...
     * // 1. we get its safety region (i.e., a square centred at the current
     * point and
     * // with width equals to twice of the point's size)
     * // 2. by querying the quadTree, we try to find if there is any existing
     * 2D-object
     ...
     * // 3. if we successfully add all the points to the quad-tree, then we
     * are sure
    ...
    private static boolean checkObjects(
        ...
        while (!sortedPoints.isEmpty()) {
          ...
          Point2D centre = point.getCenter();
          Point2D topRight = new Point2D(centre.x + point.getSize(),
                  centre.y+ point.getSize());
          Point2D bottomLeft = new Point2D(centre.x - point.getSize(),
                  centre.y - point.getSize());
          AABB region2 = new AABB(bottomLeft, topRight);
          if (!quadTree.queryRegion(region2).isEmpty()) {
            return false;
          }
          quadTree.add(point);
          sortedPoints.remove();
