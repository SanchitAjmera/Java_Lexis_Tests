...
public class NineTailsWeightedGraph {
	...
	private int calculateWeights(int index1, int index2){
		char[] conf1 = indexToConfiguration(index1);
		char[] conf2 = indexToConfiguration(index2);
		int weights = 0;
		for (int i = 0; i < 9 ; i++){
			if (conf1[i] != conf2[i]){
				weights++;
	...
	private void constructGraph() {
	    for (int i = 0; i < NUM_CONFIGURATIONS; i++){
	    	...
	    	for (int j = 0; j < NUM_CONFIGURATIONS; j++){
	    		...
	    		WeightedEdge wEdge = new WeightedEdge(i, j,
						calculateWeights(i,j));
				try {
